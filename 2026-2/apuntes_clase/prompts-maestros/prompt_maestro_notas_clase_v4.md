...
> **Revisión**: actualizado el 24 de julio de 2026 tras la sesión de auditoría de `clase7_autoevaluacion.md`. Cambios: nueva Fase 7 — Triage de observaciones externas (sección 12), con el mismo protocolo ya aplicado ese día al prompt maestro de autoevaluación (verificar vigencia → chequear contra reglas no negociables → chequear contra decisiones ya tomadas → aplicar solo si pasa las tres); nuevo ítem de checklist final referenciándola.
>
> **Revisión anterior**: 23 de julio de 2026, tras la sesión de revisión de la Clase 7 (Lógica de Predicados). Cambios: nueva subsección 8.7 que consolida las reglas de renderizado GitHub/KaTeX (las 3 ya existentes más 6 reglas nuevas: macros bloqueados como `\hphantom`, `$...$` anidado dentro de `*"..."*`, espacio pegado al `$` de cierre, `\,` interpretado como coma por CommonMark, `\underline{}` colapsando sobre contenido invisible, `$` tocando `)`/`(`/`"`/`—` directamente) y un protocolo de diagnóstico para fallos de renderizado nuevos no cubiertos por las reglas conocidas; Fase 5 punto 4 y el checklist final (sección 11) actualizados para referenciar la lista consolidada.
...
4. **Presentación y accesibilidad técnica**: no te fíes de una revisión visual — **haz una búsqueda de texto literal en todo el documento** (equivalente a `grep`) de cada una de las 9 reglas de renderizado GitHub/KaTeX listadas en **8.7**. Este tipo de error ya se ha repetido más de una vez en el mismo documento porque una revisión visual encuentra la primera ocurrencia pero no la segunda en otra sección — no lo repitas. Antes de darme el documento por terminado, confirma explícitamente en tu resumen que hiciste esta búsqueda de texto completa para las 9 reglas y cuántas coincidencias encontraste de cada una (idealmente cero). Si yo señalo un error de este tipo y te pido corregirlo, corrígelo y **de inmediato repite la búsqueda en todo el archivo** — no asumas que fue la única ocurrencia. Si el error no corresponde a ninguna de las 9 reglas conocidas, sigue el protocolo de diagnóstico al final de 8.7.
...
- Usa notación LaTeX con delimitadores `$...$` / `$$...$$` para símbolos lógicos y matemáticos, consistente con las convenciones ya usadas en el sitio del curso (`lessons/modX/claseN.md`). El KaTeX de GitHub rompe con varios patrones que en otros entornos funcionan bien — ver la lista completa de 9 reglas en **8.7**. **Esta verificación se hace por búsqueda de texto en todo el documento, no por revisión visual** — ver Fase 5, punto 4.
...
### 8.7 Reglas de renderizado GitHub/KaTeX

El KaTeX de GitHub es más frágil que un visor local o que MathJax estándar: rompe con patrones que en otros entornos funcionan sin problema, casi siempre relacionados con delimitadores `$` en contacto directo con ciertos caracteres, o con contenido "invisible". Verifica cada uno de los siguientes por **búsqueda de texto literal en todo el documento** (no revisión visual) antes de entregar — ver también Fase 5, punto 4.

1. **Guion bajo suelto en modo matemático** (`_` sin llaves): causa "Missing open brace for subscript". Usa `\square` o `\text{...}` para placeholders, nunca `_` sin escapar ni entre llaves mal formadas.
2. **`<details>` anidado dentro de un blockquote (`>`)**: debe llevar una línea `> ` en blanco inmediatamente después de `<summary>...</summary>` y otra antes de `</details>`. Sin esas líneas en blanco, el contenido interno (negritas, LaTeX) no se procesa como Markdown.
3. **Cadenas de equivalencia genéricas** ($A\equiv A_1\equiv\cdots\equiv B$): usa el bloque vertical `$$\begin{aligned}...\end{aligned}$$` en vez de una sola línea `$$...$$` con múltiples $\equiv$.
4. **Macros bloqueados por la lista blanca de GitHub**: `\hphantom` (y probablemente otros macros de tipo phantom/box) no están permitidos — GitHub muestra el error "The following macros are not allowed". Para blancos rellenables o espacios invisibles con ancho, usa `\rule{ancho}{alto}`, nunca `\hphantom` ni `\phantom`.
5. **`$...$` anidado dentro de `*"..."*`** (variable o expresión entre comillas en cursiva) no renderiza, sea en prosa o en tabla, sin importar espaciado. Si una variable suelta va dentro de `*"..."*`, escríbela sin `$` — la cursiva exterior ya la muestra en itálica (`*"x es cerveza"*`, no `*"$x$ es cerveza"*`). Los `$...$` con operadores/paréntesis siguen funcionando bien fuera de ese anidamiento.
6. **Espacio (o `\ `) pegado al `$` de cierre, por dentro**, rompe el renderizado. Si el contenido matemático termina en un espacio literal justo antes del `$` de cierre (típicamente por usar `\ ` como último carácter, ej. `$P(x):\ $`), GitHub no reconoce el delimitador. Nunca termines una expresión `$...$` con espacio pegado al `$` de cierre; si hace falta espacio después, ponlo fuera del `$`, en el Markdown normal.
7. **`\,` (espacio fino de LaTeX) se convierte en coma literal**: CommonMark interpreta `\,` como escape de coma (la coma está en su lista de puntuación escapable), y GitHub lo aplica antes de que KaTeX vea el contenido. Nunca uses `\,` para espacio fino en modo matemático — usa `\ ` (barra invertida + espacio) en su lugar, inmune a este escape porque el espacio no es puntuación escapable.
8. **`\underline{}` sobre contenido invisible colapsa**: `\underline{\rule{...}{0}}`, `\underline{\qquad...}` y variantes similares no producen una línea visible — el ancho se pierde. Para dibujar una línea o blanco para completar, usa `\rule[desplazamiento_vertical]{ancho}{alto_pequeño_no_cero}` directamente, sin envolver en `\underline{}`. Ejemplo confirmado: `\rule[-0.2em]{6em}{0.06em}`.
9. **`$` tocando directamente `)`, `(`, `"` o `—` (sin espacio) rompe el renderizado**, sea de apertura o de cierre. Los dos puntos (`:`) parecen seguros; comas y puntos no se han confirmado como problemáticos, así que no los toques salvo evidencia nueva. Nunca `($x$)`, siempre `( $x$ )`; nunca `$x$"`, siempre `$x$ "`.

**Regla de oro que resume 4-9**: siempre deja un carácter de separación (espacio) entre `$` y cualquier cosa que no sea el propio contenido matemático o texto plano simple — el problema recurrente es un delimitador `$` en contacto directo con puntuación o con contenido invisible.

**Protocolo si aparece un fallo de renderizado nuevo** (no cubierto por las 9 reglas anteriores):

1. Pide al profesor una **captura de pantalla** de cómo se ve ya renderizado en GitHub — no basta con describir el síntoma en texto; el diagnóstico de cada una de las reglas anteriores solo fue posible viendo el resultado real.
2. Antes de proponer una corrección, **verifica si el patrón roto se repite en otras partes del documento** (búsqueda exhaustiva, no solo la línea reportada) — varios de estos bugs afectaban decenas de apariciones simultáneas.
3. Si la causa no es evidente de inmediato, **prefiere un diagnóstico de variantes múltiples en una sola pasada** (una tabla con 4-8 alternativas aisladas que el profesor prueba de una vez) en vez de corregir y esperar confirmación una por una — evita el ciclo de ensayo y error.
4. Una vez confirmada la causa y la corrección, **documéntala como regla nueva** en esta misma subsección 8.7, con el mismo formato, para que no se repita el diagnóstico en clases futuras.
...
- [ ] ¿Se hizo una búsqueda de texto literal (no revisión visual) de cada una de las 9 reglas de renderizado GitHub/KaTeX de la sección 8.7, y se confirmó cero coincidencias problemáticas en cada una?
- [ ] ¿Todo `$` de apertura o cierre tiene un espacio (o texto plano) de por medio antes de tocar `)`, `(`, `"` o `—`? ¿Ninguna variable suelta dentro de `*"..."*` lleva `$`? ¿Ningún `$...$` termina en espacio pegado al `$` de cierre? ¿Se usó `\ ` en vez de `\,` para espacio fino? ¿Ningún blanco para completar usa `\underline{}` sobre contenido invisible?
...
- [ ] Si recibí observaciones de una auditoría externa sobre este documento, ¿se aplicó el triage de la Fase 7 (sección 12) antes de corregir, en vez de aplicar los cambios directamente?

---

## 12. Fase 7 — Triage de observaciones externas

Se activa cuando te traigo observaciones de una auditoría externa (otra IA, un colega, un revisor) sobre un documento ya entregado (`claseN.md` o cualquier archivo derivado de él). No apliques ninguna corrección directamente — primero clasifica cada observación:
...
2. **¿Viola alguna regla no negociable de la sección 8** (separación teoría/narrativa, fidelidad a los ejercicios reales, terminología no definida antes de su primer uso, copyright, renderizado, etc.)? Si la corrección propuesta la viola, no se aplica tal cual — repórtala como conflicto y propón, si existe, una versión alternativa que sí respete la regla.
3. **¿Contradice una decisión mía explícita ya tomada** (elección de hilo narrativo, alcance de un tema, formato de un ejercicio, nombre de una sección)? Si es así, no la apliques — señala el conflicto y explica por qué, en vez de revertir silenciosamente una decisión aprobada.
...
Para cada observación, preséntame una tabla breve (columna de diagnóstico + acción) antes de editar — no apliques cambios en bloque sin ese resumen. Si una observación queda en zona gris (señala una ambigüedad real aunque la corrección propuesta no sea la correcta), muéstrame el texto exacto de tu propuesta alternativa antes de aplicarla, no solo la idea.
