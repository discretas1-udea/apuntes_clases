...
> **Revisión**: actualizado el 23 de julio de 2026 tras la sesión de revisión de la Clase 7 (Lógica de Predicados). Cambios: nueva subsección 8.7 que consolida las reglas de renderizado GitHub/KaTeX (las 3 ya existentes más 6 reglas nuevas: macros bloqueados como `\hphantom`, `$...$` anidado dentro de `*"..."*`, espacio pegado al `$` de cierre, `\,` interpretado como coma por CommonMark, `\underline{}` colapsando sobre contenido invisible, `$` tocando `)`/`(`/`"`/`—` directamente) y un protocolo de diagnóstico para fallos de renderizado nuevos no cubiertos por las reglas conocidas; Fase 5 punto 4 y el checklist final (sección 11) actualizados para referenciar la lista consolidada.
>
> **Revisión anterior**: 19 de julio de 2026, tras la Clase 5 (Enfoque Axiomático). Cambios: numeración con tercera fuente (nombre de archivo local) y manejo de cronogramas de semestres pasados; verificación de que el contenido del sitio pertenezca al semestre actual; dos reglas de renderizado (`<details>` dentro de blockquote, cadenas de equivalencia genéricas); subsección nueva sobre diagramas Mermaid.
